			...
			try {
				...
				while (i.hasNext()) {
					...
					if (!fi.isFormField()) {
						...
						String coverPicName = email+fName;

						long sizeInBytes = fi.getSize();
						// Write the file
						if (coverPicName.lastIndexOf("\\") >= 0) {

							file = new File(filePath + coverPicName.substring(coverPicName.lastIndexOf("\\")));
						} else {

							file = new File(filePath + coverPicName.substring(coverPicName.lastIndexOf("\\") + 1));
						}
						fileName = coverPicName.substring(coverPicName.lastIndexOf("\\") + 1);
						fi.write(file);

					}

				}
				message = MySqlDataStoreUtilities.updateUserCoverPicture(fileName, user);
